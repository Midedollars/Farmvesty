...
const validatefarminvestpackage = require("../middleware/joiValidate.middleware")
const db = require("../database/connectPGsql");
const AppError = require("../utils/appError");
const { successResMsg, errorResMsg } = require("../utils/response");
// const { validatefarminvestpackage } = require("../middleware/joiValidiate.middleware");
...
const viewAllPackages = async (req, res, next) => {
  try {
    ...
    const allPackages = await db.query(
      "SELECT * FROM Packages Order By id LIMIT 10 OFFSET (1 - 1) * 10"
    );
    const count = await db.query("SELECT COUNT(*)FROM packages");
    return successResMsg(res, 200, {
      message: "Packages viewed successfully",
      count,
      viewallPackages,
    });
  } catch (error) {
